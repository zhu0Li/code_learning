...
from pos_embeddings.relative_pos_enc_qkv import Relative2DPosEncQKV
...
class AxialAttention(nn.Module):
    def __init__(self, dim, in_channels=128, heads=8, dim_head_kq=8):
        ...
        """
        super().__init__()
        self.dim_head = in_channels // heads    #每个head的通道数
        self.dim = dim
...
if __name__ == '__main__':
    input = torch.randn((5,256,5,1024))
    model =AxialAttention(dim=5*1024, in_channels=256)
    output = model(input)
    print(output.shape)