...
class HRViTAxialBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert H % window_size[0] == 0 , "H: input should be able to be splited into windows"
        assert  W>=window_size[1] and W%window_size[1] == 0, "W: input should be able to be splited into windows"
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x1 = self.attnh(x1)
        # print('3',x.shape)
        x1 = x1.permute(0, 2, 3, 1)  # BHWC
        # print('4',x.shape)
        x1 = self.attn_norm_2(x1)
        x1 = x1.permute(0, 3, 1, 2)  # BCHW
        # print('5',x.shape)
        x2 = self.attnw(x2)
...
if __name__ == '__main__':
    ...
    model_ = HRViTAxialBlock(in_dim=8,dim=8, H=32, W=5, window_size=[8,5]).to(device)
